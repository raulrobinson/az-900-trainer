...
export default function EnglishPage() {
    ...
    useEffect(() => {
        const fetchQuestions = async () => {
            ...
            try {
                setIsLoading(true);
                const response = await fetch('/api/answers-english');
                const data: Question[] = await response.json();
                setQuestions(data);
                setQuestion(data[0]);
            } catch (err) {
                setError('Do not load questions');
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="flex flex-col items-center justify-center px-2 py-2">
            <h1 className="text-2xl font-bold mb-4">AZ-900 Trainer</h1>
                ...
                <div className="w-full max-w-lg border p-4 rounded shadow-lg">
                    ...
                    <div className="mt-4 space-y-3">
                        {["a", "b", "c", "d"].map((option) => {
                            ...
                            if (!optionText || !optionValue) return null; // hidden options without text
